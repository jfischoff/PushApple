...
module PushApple.Instances where
...
instance Default DeviceToken where
    def = DeviceToken $ BS.pack $ Prelude.take 32 ([0])
            
instance Default Notification where
    def = Notification Nothing Nothing Nothing $ H.empty
